...
public class ChattServerThread extends Thread {
	...
	public ChattServerThread(ChattServer chattServer) {
		System.out.println(
				"===run ChatServerThread(chattServer)" + cal.get(Calendar.SECOND) + cal.get(Calendar.MILLISECOND));
		...
		try {
			...
			while (!isStop) {
				...
				case Protocol.MESSAGE: {
					System.out.println("===ChattServerThread protocol 200 " + cal.get(Calendar.SECOND)
					...
					showServerLog(Protocol.MESSAGE + Protocol.seperator + nickName + Protocol.seperator + message
							+ Protocol.seperator);
				}
					break;
				case Protocol.CHANGE: {
					...
					showServerLog(Protocol.CHANGE + Protocol.seperator + nickName + Protocol.seperator + afterName
				...
				case Protocol.ROOM_OUT: {
					...
					showServerLog(Protocol.ROOM_OUT + Protocol.seperator + nickName);
				}
					break run_start;
				}
			}
